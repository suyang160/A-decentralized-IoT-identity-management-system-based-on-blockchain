...
int wallet_copy_rpl(uint8_t *source, uint8_t *destination, uint32_t size,
                    int copyPos) {
    int ret_val = copyPos;
    if (size != 0) {
...
int wallet_encode_list(EncodeEthereumSignTx *new_msg, EncodeEthereumTxRequest *new_tx,
    ...
    if (totalLength < SIZE_THRESHOLD) {
        data = pvPortMalloc(1 + totalLength);
        ...
        uint8_t *lenBytes;
        lenBytes = pvPortMalloc(byteNum);
        int i;
        for (i = 0; i < byteNum; ++i) {
            lenBytes[byteNum - 1 - i] =
                    (uint8_t) ((tmpLength >> (8 * i)) & 0xFF);
        }
        data = pvPortMalloc(1 + byteNum + totalLength);
...
void wallet_encode_element(pb_byte_t *bytes, pb_size_t size,
    ...
    if (psize == 0) {
        ...
        *new_size = psize + 1;
        ...
        pb_byte_t *data = pvPortMalloc(1 + lengthOfLength + psize);
